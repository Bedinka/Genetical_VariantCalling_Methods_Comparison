...
Channel
	...
	.ifEmpty { error "Nem található egyező leolvasás: ${params.reads}" }
	.into{g_4_reads_g_3;g_4_reads_g_18}
 } else {  
	g_4_reads_g_3 = Channel.empty()
	g_4_reads_g_18 = Channel.empty()
 }

Channel.value(params.mate).into{g_5_mate_g_3;g_5_mate_g_11;g_5_mate_g_16;g_5_mate_g_18;g_5_mate_g_19;g_5_mate_g_20;g_5_mate_g_21}
...
g_4_reads_g_18.set{g_18_reads0_g_19}
g_18_log_file1_g_11 = Channel.empty()
} else {


process Adapter_Removal {

publishDir params.outdir, mode: 'copy', saveAs: {filename -> if (filename =~ /reads\/.*.fastq$/) "adapter_removal/$filename"}
input:
 set val(name), file(reads) from g_4_reads_g_18
 val mate from g_5_mate_g_18

output:
 set val(name), file("reads/*.fastq")  into g_18_reads0_g_19
 file "*.{fastx,trimmomatic}.log"  into g_18_log_file1_g_11
...
if ("!{mate}" eq "pair") {
    if ("!{Tool_for_Adapter_Removal}" eq "trimmomatic") {
        ...
        print "Fastx_clipper használata nem javasolt pair fájlok esetén.";
    }
} else {
    if ("!{Tool_for_Adapter_Removal}" eq "trimmomatic") {
...
if ("!{remove_previous_reads}" eq "true") {
    ...
    $inputsdir .= "/work";
    print "INFO: Leolvasási bemenetek törlődnek, ha a $workdir $inputsdir ba találhatóak\\n";
...
g_18_reads0_g_19.set{g_19_reads0_g_20}
g_19_log_file1_g_21 = Channel.empty()
} else {


process Trimmer {

publishDir params.outdir, mode: 'copy', saveAs: {filename -> if (filename =~ /reads\/.*q$/) "trimmer/$filename"}
input:
 set val(name), file(reads) from g_18_reads0_g_19
 val mate from g_5_mate_g_19

output:
 set val(name), file("reads/*q")  into g_19_reads0_g_20
 file "*.log" optional true  into g_19_log_file1_g_21
...
shell:
phred = params.Trimmer.phred
single_or_paired_end_reads = params.Trimmer.single_or_paired_end_reads
trim_length_5prime = params.Trimmer.trim_length_5prime
trim_length_3prime = params.Trimmer.trim_length_3prime
trim_length_5prime_R1 = params.Trimmer.trim_length_5prime_R1
trim_length_3prime_R1 = params.Trimmer.trim_length_3prime_R1
trim_length_5prime_R2 = params.Trimmer.trim_length_5prime_R2
trim_length_3prime_R2 = params.Trimmer.trim_length_3prime_R2
remove_previous_reads = params.Trimmer.remove_previous_reads
...
process Trimmer_Removal_Summary {

publishDir params.outdir, mode: 'copy', saveAs: {filename -> if (filename =~ /trimmer_summary.tsv$/) "trimmer_summary/$filename"}
input:
 file logfile from g_19_log_file1_g_21.collect()
 val mate from g_5_mate_g_21

output:
 file "trimmer_summary.tsv"  into g_21_outputFileTSV00
...
g_19_reads0_g_20.set{g_20_reads00}
g_20_log_file1_g_16 = Channel.empty()
} else {


process Quality_Filtering {

publishDir params.outdir, mode: 'copy', saveAs: {filename -> if (filename =~ /reads\/.*q$/) "quality_filter/$filename"}
input:
 set val(name), file(reads) from g_19_reads0_g_20
 val mate from g_5_mate_g_20

output:
 set val(name), file("reads/*q")  into g_20_reads00
 file "*.{fastx,trimmomatic}_quality.log" optional true  into g_20_log_file1_g_16
...
if ("!{tool}" eq "trimmomatic") {
    ...
    if ("!{mate}" eq "pair") {
        print("WARNING: Fastx opció nem megfelelő paired leolvasások esetén. Ez a lépés kihagyásra kerül.");
...
publishDir params.outdir, mode: 'copy', saveAs: {filename -> if (filename =~ /quality_filter_summary.tsv$/) "quality_filtering_summary/$filename"}
input:
 file logfile from g_20_log_file1_g_16.collect()
 val mate from g_5_mate_g_16

output:
 file "quality_filter_summary.tsv"  into g_16_outputFileTSV00
...
process Adapter_Removal_Summary {

publishDir params.outdir, mode: 'copy', saveAs: {filename -> if (filename =~ /adapter_removal_summary.tsv$/) "adapter_removal_summary/$filename"}
...
input:
 file logfile from g_18_log_file1_g_11.collect()
 val mate from g_5_mate_g_11

output:
 file "adapter_removal_summary.tsv"  into g_11_outputFileTSV00
 file "adapter_removal_detailed_summary.tsv" optional true  into g_11_outputFile11
...
input:
 val mate from g_5_mate_g_3
 set val(name), file(reads) from g_4_reads_g_3

output:
 file '*.{html,zip}'  into g_3_FastQCout00
